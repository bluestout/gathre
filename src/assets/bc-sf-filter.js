...
function buildImages(data) {
  ...
  if (data.available) {
    ...
    if (available_variant.length > 0) {
      thumbUrl = available_variant.featured_image.src;
    } else {
      if (data.selected_or_first_available_variant.featured_image.src != undefined ) {
        thumbUrl = data.selected_or_first_available_variant.featured_image.src;
      } else {
        thumbUrl = images.length > 1 ? images[0]['src'] : thumbUrl;
      }
    }
  ...
  if (bcSfFilterConfig.custom.active_image_swap) {
    ...
    for (let i = 0; i < images.length; i++) {
      ...
      if (image['src'] == thumbUrl) {
        if (images[i+1]['src'] != undefined ) {
          flipImageUrl = images[i+1]['src'];
        }
      }
    }
...
function buildPrice(data) {
  ...
  if (bcSfFilterConfig.custom.show_price) {
    ...
    if (onSale) {
      ...
      html += '<span class="bc-sf-filter-product-item-sale-price">' + bcsffilter.formatMoney(data.selected_or_first_available_variant.price) + '</span>';
    } else {
      if (priceVaries) {
        html += (bcSfFilterConfig.label_basic.from) + ' ';
      }
      html += '<span class="bc-sf-filter-product-item-regular-price">' + bcsffilter.formatMoney(data.selected_or_first_available_variant.price) + '</span>';
    }
    html += '<span class="stamped-product-reviews-badge" data-id="'+data.id+'"></span>';
    html += '</p>';
  }
...
BCSfFilter.prototype.buildDefaultElements=function(){var isiOS=/iPad|iPhone|iPod/.test(navigator.userAgent)&&!window.MSStream,isSafari=/Safari/.test(navigator.userAgent),isBackButton=window.performance&&window.performance.navigation&&2==window.performance.navigation.type;if(!(isiOS&&isSafari&&isBackButton)){var self=this,url=window.top.location.href.split("?")[0],searchQuery=self.isSearchPage()&&self.queryParams.hasOwnProperty("q")?"&q="+self.queryParams.q:"";window.location.replace(url+"?view=bc-original"+searchQuery)}};
